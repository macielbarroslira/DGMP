...
# Renomeia as colunas para nomes de variáveis amigáveis (minúsculas, sem espaços)
...
# Cria as listas de opções para os filtros
...
app.layout = html.Div(style={'backgroundColor': '#f4f6f9', 'fontFamily': 'Segoe UI, Arial, sans-serif'}, children=[
    ...
    html.Div([
        ...
        # CORREÇÃO: Placeholders atualizados para os nomes corretos
        html.Div([
        ...
        html.Div([
            ...
            html.Div(dcc.Dropdown(id='filtro-exercicio', options=options_exercicio, multi=True, placeholder="Exercício"), style=filter_style),
    ...
    html.Div([
        html.H4("Tabela de Detalhes", style={'textAlign': 'center'}),
        dash_table.DataTable(
            id='tabela-detalhes',
            # CORREÇÃO: Nomes dos cabeçalhos da tabela definidos explicitamente
            columns=[
                {'name': 'Município', 'id': 'municipio'}, {'name': 'Exercício', 'id': 'exercicio'},
                {'name': 'Fase', 'id': 'fase'}, {'name': 'Instrumento', 'id': 'instrumento'},
                {'name': 'Situação', 'id': 'situacao'}, {'name': 'Região de Saúde', 'id': 'regiao_saude'},
                {'name': 'Macrorregião', 'id': 'macrorregiao'}
            ],
...
def generate_dynamic_title_suffix(filters):
    parts = []
    max_items_to_list = 2
    # CORREÇÃO: Dicionário com os nomes corretos para exibição nos títulos
    filter_labels = {
